...
var MVModalTwoFactorAuthentication = (function () {
	...
	var whatsappWrapper;
    var whatsappForm;
    var whatsappSubmitButton;
    var whatsappCancelButton;
    var whatsappValidator;
    ...
    var handleOptionsForm = function () {
        ...
        optionsSelectButton.addEventListener("click", function (e) {
            e.preventDefault();
            var option = optionsWrapper.querySelector(
                '[name="auth_option"]:checked'
            );
            ...
            switch (option.value) {
				...
				case "email":
					...
					break;
				case "whatsapp":
					whatsappWrapper.classList.remove("d-none");
					break;
                default:
                    break;
            }
    ...
    var showOptionsForm = function () {
		...
		whatsappWrapper.classList.add("d-none");
    };
            ...
            if (smsValidator) {
                smsValidator.validate().then(function (status) {
                    ...
                    if (status == "Valid") {
                        ...
                        var mobileValue =
                            smsForm.querySelector('[name="mobile"]').value;
                            ...
                            if (value.isConfirmed) {
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        smsSubmitButton.removeAttribute(
                                            "data-mv-indicator"
                                        );
                                        ...
                                        $(
                                            '[data-mv-element="code-form"] #type'
                                        ).val("email");
	...
	var handleWhatsappForm = function () {
        // Init form validation rules. For more info check the FormValidation plugin's official documentation:https://formvalidation.io/
        whatsappValidator = FormValidation.formValidation(whatsappForm, {
            fields: {
                mobile: {
                    validators: {
                        notEmpty: {
        ...
        // Handle apps submition
        whatsappSubmitButton.addEventListener("click", function (e) {
            e.preventDefault();

            // Validate form before submit
            if (whatsappValidator) {
                whatsappValidator.validate().then(function (status) {
                    console.log("validated!");

                    if (status == "Valid") {
                        // Show loading indication
                        whatsappSubmitButton.setAttribute("data-mv-indicator", "on");
                        ...
                        // Get the input field within the emailForm
                        var mobileValue =
						whatsappForm.querySelector('[name="mobile"]').value;
                        ...
                        Swal.fire({
                            ...
                            if (value.isConfirmed) {
                                $.ajax({
                                    type: whatsappForm.method,
                                    url: whatsappForm.action,
                                    headers: {
                                        "X-CSRF-TOKEN": $(
                                            'meta[name="csrf-token"]'
                                        ).attr("content"),
                                    ...
                                    success: function (data) {
                                        whatsappSubmitButton.removeAttribute(
                                            "data-mv-indicator"
                                        );
        ...
        // Handle whatsapp cancelation
        whatsappCancelButton.addEventListener("click", function (e) {
            e.preventDefault();
            var option = optionsWrapper.querySelector(
                '[name="auth_option"]:checked'
            );
        ...
        emailSubmitButton.addEventListener("click", function (e) {
            ...
            if (emailValidator) {
                emailValidator.validate().then(function (status) {
                    ...
                    if (status == "Valid") {
                        emailSubmitButton.setAttribute(
                            "data-mv-indicator",
                            "on"
                        );
                        ...
                        // Get the input field within the emailForm
                        var emailValue = emailForm.querySelector(
                            '[name="emailAddress"]'
                        ).value;
                        ...
                        Swal.fire({
                            ...
                            if (value.isConfirmed) {
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        emailSubmitButton.removeAttribute(
                                            "data-mv-indicator"
                                        );
    ...
    var handleCodeForm = function () {
        // Init form validation rules. For more info check the FormValidation plugin's official documentation:https://formvalidation.io/
        codeValidator = FormValidation.formValidation(codeForm, {
            fields: {
                code: {
                    validators: {
                        notEmpty: {
            ...
            if (codeValidator) {
                codeValidator.validate().then(function (status) {
                    ...
                    if (status == "Valid") {
                        ...
                        // Get the input field within the emailForm
                        var typeValue =
                            codeForm.querySelector('[name="type"]').value;
                        var codeValue =
                            codeForm.querySelector('[name="code"]').value;
                        ...
                        Swal.fire({
                            ...
                            if (value.isConfirmed) {
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        codeSubmitButton.removeAttribute(
                                            "data-mv-indicator"
                                        );
    ...
    return {
        init: function () {
            ...
            modal = document.querySelector(
                "#mv_modal_two_factor_authentication"
            );
            ...
            optionsSelectButton = modal.querySelector(
                '[data-mv-element="options-select"]'
            );

            smsWrapper = modal.querySelector('[data-mv-element="sms"]');
            smsForm = modal.querySelector('[data-mv-element="sms-form"]');
            smsSubmitButton = modal.querySelector(
                '[data-mv-element="sms-submit"]'
            );
            smsCancelButton = modal.querySelector(
                '[data-mv-element="sms-cancel"]'
            );

            emailWrapper = modal.querySelector('[data-mv-element="email"]');
            emailForm = modal.querySelector('[data-mv-element="email-form"]');
            emailSubmitButton = modal.querySelector(
                '[data-mv-element="email-submit"]'
            );
            emailCancelButton = modal.querySelector(
                '[data-mv-element="email-cancel"]'
            );

            codeWrapper = modal.querySelector('[data-mv-element="code"]');
            codeForm = modal.querySelector('[data-mv-element="code-form"]');
            codeSubmitButton = modal.querySelector(
                '[data-mv-element="code-submit"]'
            );
            codeCancelButton = modal.querySelector(
                '[data-mv-element="code-cancel"]'
            );


			whatsappWrapper = modal.querySelector('[data-mv-element="whatsapp"]');
            whatsappForm = modal.querySelector('[data-mv-element="whatsapp-form"]');
            whatsappSubmitButton = modal.querySelector(
                '[data-mv-element="whatsapp-submit"]'
            );
            whatsappCancelButton = modal.querySelector(
                '[data-mv-element="whatsapp-cancel"]'
            );
